...
1. Скопируйте `.env.example` в `.env` и при необходимости измените значения.
   ...
   Флаг `NEXT_PUBLIC_ENABLE_TAGS` управляет отображением поля тегов в админке.
...
Поле тегов отображается при `NEXT_PUBLIC_ENABLE_TAGS=true`. Теги подгружаются
из `/api/v1/tags/`. Если запрос завершился ошибкой, форма остаётся доступной, а
под инпутом появляется предупреждение «⚠️ Теги не загрузились».
...
В формах создания и редактирования постов используется компонент **TiptapEditor**.
Доступные инструменты форматирования:

- полужирный, курсив, подчёркивание, зачёркивание;
- выбор цвета текста и фона;
- заголовки, списки, цитаты;
- блоки кода и горизонтальные линии;
- вставка ссылок;
- вставка изображений с предварительным просмотром, указанием `alt` и ширины;
- вставка слайдера изображений с настройкой задержки и автопрокрутки.

### Краткое описание кнопок

| Кнопка | Назначение |
| ------ | ---------- |
| **B** | Сделать текст полужирным |
| *I* | Курсивное начертание |
| U | Подчёркнутый текст |
| S | Зачёркнутый текст |
| "A" с цветом | Изменить цвет текста |
| Маркер | Изменить цвет фона |
| H | Заголовок второго уровня |
...
| Слайдер | Создание блока слайдера изображений |

Все подсказки и сообщения в интерфейсе редактора отображаются на русском языке.
