...
Главная страница фронтенда реализована в `frontend/src/app/page.tsx` и стилизована
в духе темы Read WP. Используется центрированная колонка шириной до 780&nbsp;px,
минималистичная навигация на сером фоне и карточки постов.
Сейчас выводятся мок-данные, но структура и стили готовы к подключению API.
Типографика основана на шрифтах **Lora** и **Coustard**. Coustard подключён через `next/font` только для латинского набора с весами **400** и **900**, для кириллицы используется fallback **Inter**.
